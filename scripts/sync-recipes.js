...
function nutritionGuidance(recipe) {
  ...
  if (recipe.healthScore < (sweet ? 10 : 15)) {
    notes.push("The provider health score is lower than CookWise's preferred range; balance this meal with nutrient-rich foods.");
  }
  ...
  if (nutrition.fat > (sweet ? 28 : 45)) {
    notes.push("Fat is relatively high per serving; portion size and lower-fat substitutions may help.");
  }
  ...
  if (nutrition.sodium > 1000) {
    notes.push("Sodium is relatively high; lower-sodium packaged ingredients or less added salt may help.");
  }
...
function saveRecipes(database, recipes) {
  const statement = database.prepare(`
    INSERT INTO recipes (
      ...
      ingredients_json, steps_json, nutrition_json, synced_at
    ) VALUES (
      ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
    )
...
async function syncRecipes() {
  ...
  if (recipes.length === 0) {
    throw new Error(
      "No recipes passed the simplicity and smart-eating filters. No database changes were made."
    );
  }
